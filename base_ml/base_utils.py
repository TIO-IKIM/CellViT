...
def filter2D(input_tensor: torch.Tensor, kernel: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        torch.Tensor: The convolved tensor of the same shape as the input.
    """
...
def gaussian(
    ...
    Args:
        window_size (int): Number of elements for the output tensor.
        sigma (float): Std of the gaussian distribution.
        device (torch.device): Device for the tensor.

    Returns:
        torch.Tensor: A gaussian 1D tensor. Shape: (window_size, ).
    """
...
def gaussian_kernel2d(
    ...
    Args:
        window_size (int): Number of rows and columns for the output tensor.
        sigma (float): Std of the gaussian distribution.
    ...
    Returns:
        torch.Tensor: A tensor of shape (1, 1, window_size, window_size)
    """
...
def sobel_hv(window_size: int = 5, device: torch.device = None):
    ...
    Returns:
        torch.Tensor: The computed 1st order derivatives of the input tensor. Shape (B, 2, H, W)
