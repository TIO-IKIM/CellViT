...
from cell_segmentation.trainer.trainer_unetr2d import UNETR2dTrainer
...
class ExperimentUNETR2d(BaseExperiment):
    ...
    def get_loss_fn(self, loss_fn_settings: dict) -> dict:
        ...
        Args:
            ...
            For further information, please have a look at the file configs/examples/cell_segmentation/unetr2d.yaml
    ...
    def get_scheduler(self, scheduler_type: str, optimizer: Optimizer) -> _LRScheduler:
        """Get the learning rate scheduler for UNETR2d
    ...
    def get_datasets(
        ...
        if "val_split" in self.run_conf["data"]:
            ...
            train_dataset, val_dataset = torch.utils.data.random_split(
                full_dataset, lengths=[0.9, 0.1], generator=generator_split
            )
    ...
    def get_train_model(
        ...
        """Return the UNETR2d training model
        ...
        Returns:
            UNETR2d: UNETR2d training model with given setup
        """
        ...
        if backbone_type.lower() == "default":
            ...
            if pretrained_model is not None:
                self.logger.info(
                    f"Loading pretrained UNETR2D model from path: {pretrained_model}"
                )
                unetr2d_pretrained = torch.load(pretrained_model)
                self.logger.info(model.load_state_dict(unetr2d_pretrained, strict=True))
                self.logger.info("Loaded UNETR2D model")
        ...
        if backbone_type == "ViT256":
            ...
            if pretrained_model is not None:
                ...
                unetr2d_pretrained = torch.load(pretrained_model, map_location="cpu")
                self.logger.info(model.load_state_dict(unetr2d_pretrained, strict=True))
            model.freeze_encoder()
            self.logger.info("Loaded UNETR2D-Vit256 model")
        if backbone_type in ["SAM-B", "SAM-L", "SAM-H"]:
            ...
            if pretrained_model is not None:
                ...
                self.logger.info(model.load_state_dict(unetr2d_pretrained, strict=True))
            model.freeze_encoder()
            self.logger.info(f"Loaded UNETR2D-SAM model with backbone: {backbone_type}")
    ...
    def get_transforms(
        ...
        configs/examples/cell_segmentation/unetr2d.yaml
