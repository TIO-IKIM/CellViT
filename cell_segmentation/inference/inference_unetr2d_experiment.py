# -*- coding: utf-8 -*-
# UNETR2d Inference Method for Patch-Wise Inference on a test set
# Without merging WSI
#
...
class InferenceUNETR2d:
    ...
    def setup_patch_inference(
        ...
        Returns:
            tuple[Union[UNETR2d, UNETR2dUnshared, UNETR2dVIT256, UNETR2dVIT256Unshared, UNETR2dSAM, UNETR2dSAMUnshared], DataLoader, dict]:
                Union[UNETR2d, UNETR2dUnshared, UNETR2dVIT256, UNETR2dVIT256Unshared, UNETR2dSAM, UNETR2dSAMUnshared]: Best model loaded form checkpoint
    ...
    def run_patch_inference(
        ...
        Args:
            model (Union[UNETR2d, UNETR2dUnshared, UNETR2dVIT256, UNETR2dVIT256Unshared, UNETR2dSAM, UNETR2dSAMUnshared]): Model to use for inference
    ...
    def inference_step(
        ...
        Args:
            model (UNETR2d): Model to use for inference
        ...
        # legacy reasons for storing and testing with old eval code
        # for idx, img_name in enumerate(image_names):
        # outname_pred = (
        #     Path(
        #         "/homes/fhoerst/histo-projects/DigitalHistologyHub/outputs/cell_seg_runs/baseline-results/ViT256/2023-05-17T153418_UNetr2D-Vit-256-baseline-fold-3/converted_predictions/pred"
        #     )
        #     / f"{Path(img_name).stem}.mat"
        # )
        # outname_gt = (
        #     Path(
        #         "/homes/fhoerst/histo-projects/DigitalHistologyHub/outputs/cell_seg_runs/baseline-results/ViT256/2023-05-17T153418_UNetr2D-Vit-256-baseline-fold-3/converted_predictions/gt"
        #     )
        #     / f"{Path(img_name).stem}.mat"
        # )

        # pred_centroid = predictions["instance_types"][idx]
        # pred_centroids = np.array([v["centroid"] for v in pred_centroid.values()])
        # pred_inst_type = np.array([v["type"] for v in pred_centroid.values()])

        # gt_centroid = gt["instance_types"][idx]
        # gt_centroids = np.array([v["centroid"] for v in gt_centroid.values()])
        # gt_inst_type = np.array([v["type"] for v in gt_centroid.values()])
        ...
        #         "inst_map": predictions["instance_types_nuclei"][idx][..., 1:]
        #         .detach()
        #         .cpu()
        #         .numpy(),
        #     },
        ...
        #     outname_gt,
        #     {
        #         "inst_centroid": gt_centroids,
        #         "inst_type": gt_inst_type,
        #         "inst_map": gt["instance_types_nuclei"][idx][..., 1:]
        #         .detach()
        #         .cpu()
        #         .numpy(),
        #     },
...
if __name__ == "__main__":
    configuration_parser = InferenceUNETR2dParser()
    configuration = configuration_parser.parse_arguments()
    print(configuration)
    inf = InferenceUNETR2d(
